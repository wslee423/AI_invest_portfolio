...
**목표**: 보안 점검 + 성능 최적화 + 최종 배포 + 포트폴리오 고도화 (선택사항).
...
### 5-6. 포트폴리오 고도화 (선택사항)
- [ ] 다중 포트폴리오 시나리오 생성 (절세우선 / 수익우선 / 균형형)
- [ ] 계좌 유형 모델 추가 (일반계좌 / ISA / IRP / 연금저축펀드)
- [ ] 절세효과 시뮬레이션 로직
- [ ] 시나리오별 비교 차트·테이블
- [ ] OpenAI 프롬프트 다중 시나리오 생성으로 확장
